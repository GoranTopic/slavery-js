...
class Service {
    ...
    getServiceListeners() {
        ...
        let listeners = [{
                ...
                callback: (number_of_nodes) => ({ result: this.nodes?.spawnNodes('slave_' + this.name, number_of_nodes, { metadata: { host: this.nm_host, port: this.nm_port } }) })
            }, {
                event: '_kill_node',
                params: ['node_id'],
                callback: async (node_ids) => {
                    ...
                    if (node_ids === undefined) {
                        ...
                        for (let i = 0; i < node_ids; i++)
                            await this.nodes?.killNode();
                    }
    ...
    handle_request(l, type) {
        ...
        return async (data) => {
            if (this.slaveMethods === undefined)
                throw new Error('Slave Methods are not defined');
    ...
    async handle_peer_discovery() {
        if (this.peerDiscoveryAddress === undefined)
            throw new Error('Peer Discovery Address is not defined');
    ...
    exit() {
        ...
        setTimeout(() => {
            ...
            if (this.processBalancer)
                this.processBalancer.exit();
            // request queue will be closed
            if (this.requestQueue)
                this.requestQueue.exit();
            // if peer discovery is defined we will close it
            if (this.peerDiscovery)
                this.peerDiscovery.exit();
