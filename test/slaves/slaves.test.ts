...
let master_callback = async ({service_slaves}: any) => {
...
let slave_callbacks = {
    'setup': (params: any, { service_slave }: any ) => {
    ...
    'run': async (wating_time: number, { service_slave }: any) => {
        // count sum of numbers
        let make_timeout = service_slave['wait_function'];
    ...
    'clean up': async (params: any, { service_slave }: any) => {
        service_slave['wait_function'] = undefined
    },
}
