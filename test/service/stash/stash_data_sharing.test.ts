...
// this test will check if the stash obj used to share data between the master and the slaves
// if working correcly
...
let stasher_service = new Service({
    ...
    mastercallback: async ({ self }) => {
        // testing if we can set data
        await self.set('test', { some_data: ['some', 'list', 'of', 'data'] })
    },
    slaveMethods: {
